...
from config import RAG_ENGINE_VERSION
...
def log_llm_usage(query_uuid: str, query: str, step: str, input_tokens: int, output_tokens: int, provider: str):
    ...
    """
    _ensure_csv_headers(TELEMETRY_FILE, ["Timestamp", "UUID", "Query", "Step", "Input Tokens", "Output Tokens", "Provider", "Version"])
...
def log_endpoint_latency(endpoint: str, latency: float, query_uuid: str):
    ...
    """
    _ensure_csv_headers(LATENCY_FILE, ["Timestamp", "Endpoint", "Latency (s)", "UUID", "Version"])
